...
import TercerosSiniestroPoliza from './TercerosSiniestroPoliza';

const { Option } = Select;

const AcuerdoConInvolucradoPoliza = ({ Id_SiniestroApasar }) => {
    ...
    const onFinish = async (values) => {
        ...
        const response = await axios.post('http://localhost:8000/AcuerdosConInvolucradoPolizas/RegistrarAcuerdosConInvolucrado', jsonData, {
        headers: {
    ...
    return (
        <div style={{ backgroundColor: '#2e1437', padding: '20px', borderRadius: '8px', textAlign: 'center' }}>
            {currentStep === 1 && (
                <>
                    <h1 className="title">Acuerdo con el involucrado en la poliza</h1>
                    <div style={{ textAlign: 'center', display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%', marginTop: '2rem', marginBottom: '2rem' }}>
                        <img src="https://cdn-icons-png.flaticon.com/512/2838/2838110.png" style={{ maxHeight: '150px' }} alt="Gastos.jpg"></img>
                    </div>
            ...
            {currentStep === 2 && (
                <div>
                    <TercerosSiniestroPoliza Id_SiniestroApasar={Id_SiniestroApasar} />
                </div>
            )}
        </div>
    );
...
export default AcuerdoConInvolucradoPoliza;
